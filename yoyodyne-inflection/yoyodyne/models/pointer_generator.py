...
class PointerGeneratorLSTMEncoderDecoder(
    ...
    def decode_step(
        ...
        # If we predict UNK (Index = 0), then we get the source position with the highest 
        # attention weight
        return torch.log(scaled_output_dist + scaled_ptr_dist), (h, c), attention_weights
    ...
    def decode(
        ...
        for t in range(num_steps):
            ...
            output, decoder_hiddens, attention_weights = self.decode_step(
                decoder_input,
                decoder_hiddens,
                source_indices,
                source_enc,
        ...
        all_attention_weights = torch.stack(all_attention_weights).transpose(0, 1)
    ...
    def forward(
        ...
        Returns:
            torch.Tensor.
        """
        print(batch.source.padded)
        row_indices = torch.nonzero(batch.source.padded == 80, as_tuple=False)
        print(row_indices)
        row_indices = torch.nonzero(batch.source.padded == 79, as_tuple=False)
        print(row_indices)
        row_indices = torch.nonzero(batch.source.padded == 78, as_tuple=False)
        print(row_indices)
        print(batch.source.padded.shape)
        count_2s = (batch.source.padded == 0).sum().item()
        print("Number of 0s in the tensor:", count_2s)
        count_2s = (batch.features.padded == 0).sum().item()
        print("Number of 0s in the tensor:", count_2s)
        print(batch.features.padded)
        ...
        if not self.has_features_encoder:
            if self.beam_width is not None and self.beam_width > 1:
                ...
                predictions, all_attention_weights = self.decode(
                    source_encoded,
                    batch.source.mask,
                    batch.source.padded,
                    last_hiddens,
...
class PointerGeneratorTransformerEncoderDecoder(
    ...
    def decode_step(
        ...
        scaled_ptr_dist = ptr_dist * (1 - gen_probs)
        scaled_output_dist = output_dist * gen_probs
        # ISSUE?: Token that are UNK, cannot be generated with this
...
def _decode(
    ...
    for idx in indices.cpu().numpy():
        yield [
            symbol_map.symbol(c)
            for c in idx
            if c not in {0, 1, 2, 3}  # HARDCODED: Special Tokens
        ]
