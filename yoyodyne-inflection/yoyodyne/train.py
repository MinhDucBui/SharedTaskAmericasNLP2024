...
def _get_logger(experiment: str, model_dir: str, log_wandb: bool) -> List:
    ...
    if log_wandb:
        trainer_logger.append(
            loggers.WandbLogger(project=experiment, log_model="False")
        )
        # Tells PTL to log best validation acc
        wandb.define_metric("bleu", summary="max")
        wandb.define_metric("chrf", summary="max")
...
def _get_callbacks(dirpath, save_top_k: int, patience: Optional[int] = None, 
                   enable_checkpointing: bool = True, monitor_metric: str = "acc",
                   every_n_train_steps: int = None, every_n_epochs: int = 1) -> List:
    ...
    elif "chrf" == monitor_metric:
        filename = "model-{chrf:.3f}"
    elif "acc" == monitor_metric:
        filename = "model-{acc:.3f}"
    ...
    if enable_checkpointing:
        ...
        trainer_callbacks += [callbacks.ModelCheckpoint(
...
def get_trainer_from_argparse_args(
    ...
    Returns:
        pl.Trainer.
    """
    dirpath = args.model_dir + "/" + args.experiment
    if args.check_val_every_n_epoch == 0:
        args.check_val_every_n_epoch = None
    return pl.Trainer.from_argparse_args(
        args,
        callbacks=_get_callbacks(dirpath, args.save_top_k, args.patience, 
                                 args.enable_checkpointing, args.monitor_metric,
                                 args.early_every_n_train_steps, args.early_every_n_epochs),
...
def get_datamodule_from_argparse_args(
    ...
    if args.checkpoint:
        root_index_path = os.path.dirname(args.checkpoint)
        index = data.Index.read(root_index_path, None)
    else:
        index = None
    datamodule = data.DataModule(
        ...
        lower_casing=args.lower_casing,
...
def get_model_from_argparse_args(
    ...
    val_targets = ["".join(example[2]) for example in datamodule.val_dataloader().dataset.samples]
    # Please pass all arguments by keyword and keep in lexicographic order.
    return model_cls(
        arch=args.arch,
        model_dir=args.model_dir + "/" + args.experiment,
        val_targets=val_targets, #HACK: For Validation
        ...
        source_mapping=datamodule.index.source_map,
        target_mapping=datamodule.index.target_map,
...
def train(
    ...
    # TODO: feels flimsy.
    if type(ckp_callback) is callbacks.ModelCheckpoint:
        return ckp_callback.best_model_path
    else:
        return None


def load_model_from_checkpoint(model, datamodule, args):
    if args.checkpoint:
        model = model.load_from_checkpoint(args.checkpoint)
        model_dir = args.model_dir + "/" + args.experiment
        model.model_dir = model.get_next_versioned_filename(model_dir)
        val_targets = ["".join(example[2]) for example in datamodule.val_dataloader().dataset.samples]
        model.val_targets = val_targets # HACK: For Validation
        ...
        model.features_encoder.dropout_layer = nn.Dropout(p=args.dropout, inplace=False)
        model.source_encoder.dropout_layer = nn.Dropout(p=args.dropout, inplace=False)
        model.dropout_layer = nn.Dropout(p=args.dropout, inplace=False)
        model.decoder.dropout_layer = nn.Dropout(p=args.dropout, inplace=False)
...
def add_argparse_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--all_features', 
                        action='store_true', 
                        help='This is a hack to create models during pretraining that have the same feature amount as during finetuning')
...
def main() -> None:
    ...
    model = load_model_from_checkpoint(model, datamodule, args)
